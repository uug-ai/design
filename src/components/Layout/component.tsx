    ...
    <div className="app-container">
      <header className="header">
        <div className="header-content">
          ...
          <nav id="primary-navigation" className={`nav ${isMenuOpen ? 'nav-open' : ''}`} aria-label="Primary navigation">
            {navItems.map((item) => (
              <Link
                key={item.path}
                to={item.path}
            ...
            <a href={storybookUrl} className="nav-link" target="_blank" rel="noreferrer" onClick={() => setIsMenuOpen(false)}>
              Storybook
            </a>
          </nav>
          <div className="header-actions">
            <a
              ...
              title="GitHub"
            >
              <svg className="header-icon-svg" height="24" fill="currentColor" viewBox="3 3 18 18" aria-hidden="true">
